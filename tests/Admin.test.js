...
describe('Admin Tests', () => {
  ...
  test('Only Admin Can Get Users List', async (done) => {
    const adminLoginRes = await request(server).post('/users/login').send(userAdminMock);
    ...
    const getAllUsersNoAdminRes = await request(server).get('/api/v1/users').set('authorization', `Bearer token`);
  ...
  test('Passwords must be stored with hash', async (done) => {
    const registerResponse = await request(server).post('/users/register').send(userRegisterMock);
