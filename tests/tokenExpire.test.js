...
describe('Token expiration and refresh test', () => {
  ...
  test('Access Token expires after 10s and can be renewed with refresh token', async (done) => {
    const registerResponse = await request(server).post('/users/register').send(userMock);
    ...
    const serverResponse1 = await request(server).post('/users/token').send({ blabla: 'no token' });

    expect(serverResponse1.status).toBe(401);

    const serverResponse2 = await request(server).post('/users/token').send({ token: 'not valid token' });

    expect(serverResponse2.status).toBe(403);

    const data = await request(server).post('/users/token').send({ token: loginRes.body.refreshToken });
